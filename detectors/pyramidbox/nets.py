...
class PyramidBoxNet(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            self.priors_boxes = PriorBox(size)
            # feature1 = np.array(feature_maps)
            # feature2 = torch.from_numpy(feature1).to(self.device)

            self.priors = self.priors_boxes.forward(feature_maps)
            # self.priors = priors_boxes.forward(feature2)
